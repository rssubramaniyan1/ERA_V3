...
def get_normalization_layer(norm_type, num_features, num_groups=None, input_shape=None):
    """
    ...
    Args:
        ...
        input_shape: Shape of the input tensor (only used if norm_type is 'layer')
    Returns:
        Normalization layer
    """
    if norm_type == NormalizationTypes.BATCH:
        ...
        return nn.LayerNorm([num_features,28,28])  # Exclude batch dimension
    elif norm_type == NormalizationTypes.GROUP:
        if num_groups is None:
            num_groups = 2  # Default to 2 groups if not specified
