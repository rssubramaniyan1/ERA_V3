...
class Net(nn.Module):
    def __init__(self, norm_type=NormalizationTypes.BATCH):
        ...
        self.transition1 = nn.Sequential(
            nn.Conv2d(in_channels=16, out_channels=8, kernel_size=1, bias=False),  # Reduce channels
            nn.ReLU(),
            nn.MaxPool2d(2, 2)  # Downsample spatial dimensions
        )
        ...
        self.transition2 = nn.Sequential(
            nn.Conv2d(in_channels=24, out_channels=8, kernel_size=1, bias=False),  # Reduce channels
            nn.ReLU(),
            nn.MaxPool2d(2, 2)  # Downsample spatial dimensions
        )
        ...
        self.conv3 = nn.Sequential(
            ...
            nn.Conv2d(in_channels=10, out_channels=16, kernel_size=3, stride=1, padding=1, bias=False),
            nn.ReLU(),
            get_normalization_layer(norm_type, 16, 4, [16, 7, 7]),
            nn.Dropout(0.05)
        )
        ...
        # self.flatten_size = 16 * 7 * 7  # Channels * Height * Width
